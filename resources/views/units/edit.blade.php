...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC] mb-2">Edit Unit</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">Update unit information</p>
        </div>
            ...
            <div class="p-8">
                <form action="{{ route('units.update', $unit) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="discipline_id" id="discipline_id" required
                            ...
                            @foreach($disciplines as $discipline)
                                <option value="{{ $discipline->id }}" {{ old('discipline_id', $unit->discipline_id) == $discipline->id ? 'selected' : '' }}>
                                    {{ $discipline->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('units.show', $unit) }}" class="px-6 py-3 rounded-lg font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors flex items-center">
