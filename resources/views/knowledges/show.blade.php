...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">{{ Str::limit($knowledge->name, 100) }}</h1>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Knowledge Details</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('knowledges.edit', $knowledge) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                    Edit
                </a>
                ...
                <div class="p-6">
                    ...
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $knowledge->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Unit</dt>
                            <dd class="mt-1 text-sm text-[#1b1b18] dark:text-[#EDEDEC]">
                                <a href="{{ route('units.show', $knowledge->unit) }}" class="text-blue-600 dark:text-blue-400 hover:underline">
                                    {{ $knowledge->unit->name }}
                                </a>
                            </dd>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $knowledge->bnccs->count() }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="mt-6 bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($knowledge->bnccs as $bncc)
                            <div class="border border-gray-200 dark:border-gray-700 rounded p-4">
                                <h3 class="font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{{ $bncc->code }}</h3>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $bncc->stage->value }}</p>
