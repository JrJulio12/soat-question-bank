...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Series</h1>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Manage all series</p>
            </div>
            <a href="{{ route('series.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                Create Serie
            </a>
        </div>
                                ...
                                @foreach($series as $serie)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('series.show', $serie) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3">View</a>
                                            <a href="{{ route('series.edit', $serie) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-3">Edit</a>
                                            <form action="{{ route('series.destroy', $serie) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this serie?');">
                                                @csrf
                                                @method('DELETE')
