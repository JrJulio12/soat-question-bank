...
<div class="py-12">
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Create Question</h1>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Add a new question to the system</p>
        </div>
                ...
                <form action="{{ route('questions.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="stem" id="stem" rows="4" required
                            class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-[#0a0a0a] dark:text-[#EDEDEC] shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('stem') border-red-500 @enderror">{{ old('stem') }}</textarea>
                        @error('stem')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="answer_text" id="answer_text" rows="3"
                            class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-[#0a0a0a] dark:text-[#EDEDEC] shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('answer_text') border-red-500 @enderror">{{ old('answer_text') }}</textarea>
                        @error('answer_text')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="max-h-48 overflow-y-auto border border-gray-300 dark:border-gray-700 rounded-md p-2">
                            @foreach($bnccs as $bncc)
                                <label class="flex items-center mb-2">
                                    <input type="checkbox" name="bnccs[]" value="{{ $bncc->id }}" {{ in_array($bncc->id, old('bnccs', [])) ? 'checked' : '' }}
