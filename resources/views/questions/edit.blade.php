...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC] mb-2">Edit Question</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">Update question information</p>
        </div>
                ...
                <form action="{{ route('questions.update', $question) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="stem" id="stem" rows="5" required
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-[#0a0a0a] dark:text-[#EDEDEC] shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all resize-none @error('stem') border-red-500 focus:ring-red-500/20 @enderror">{{ old('stem', $question->stem) }}</textarea>
                        @error('stem')
                            ...
                            <div>
                                ...
                                <select name="bnccs[]" id="bnccs" multiple class="multiselect w-full">
                                    @foreach($bnccs as $bncc)
                                        <option value="{{ $bncc->id }}" {{ in_array($bncc->id, old('bnccs', $question->bnccs->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $bncc->code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="subjects[]" id="subjects" multiple class="multiselect w-full">
                                    @foreach($subjects as $subject)
                                        <option value="{{ $subject->id }}" {{ in_array($subject->id, old('subjects', $question->subjects->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $subject->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('questions.show', $question) }}" class="px-6 py-3 rounded-lg font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors flex items-center">
