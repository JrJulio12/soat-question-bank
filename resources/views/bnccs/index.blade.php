...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">BNCCs</h1>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Manage all BNCCs</p>
            </div>
            <a href="{{ route('bnccs.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                Create BNCC
            </a>
        </div>
        ...
        <div class="mb-4">
            <form method="GET" action="{{ route('bnccs.index') }}" class="flex gap-3 items-end">
                <div class="flex-1">
        ...
        <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                @if($bnccs->isEmpty())
                    <p class="text-gray-500 dark:text-gray-400">No BNCCs found.</p>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Series</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Knowledges</th>
                            ...
                            <tbody class="bg-white dark:bg-[#161615] divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($bnccs as $bncc)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ $bncc->knowledges->count() }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('bnccs.show', $bncc) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3">View</a>
                                            <a href="{{ route('bnccs.edit', $bncc) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-3">Edit</a>
                                            <form action="{{ route('bnccs.destroy', $bncc) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this BNCC?');">
                                                @csrf
                                                @method('DELETE')
