...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC] mb-2">Create BNCC</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">Add a new BNCC to the system</p>
        </div>

        <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-lg sm:rounded-xl border border-gray-200 dark:border-gray-700">
            <div class="p-8">
                <form action="{{ route('bnccs.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-[#0a0a0a] dark:text-[#EDEDEC] shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all resize-none @error('description') border-red-500 focus:ring-red-500/20 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400 flex items-center animate-pulse">
                                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                            ...
                            <div>
                                ...
                                <select name="series[]" id="series" multiple class="multiselect w-full">
                                    @foreach($series as $serie)
                                        <option value="{{ $serie->id }}" {{ in_array($serie->id, old('series', [])) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="knowledges[]" id="knowledges" multiple class="multiselect w-full">
                                    @foreach($knowledges as $knowledge)
                                        <option value="{{ $knowledge->id }}" {{ in_array($knowledge->id, old('knowledges', [])) ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('bnccs.index') }}" class="px-6 py-3 rounded-lg font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors flex items-center">
