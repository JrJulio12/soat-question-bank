...
<div class="py-12">
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Edit BNCC</h1>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Update BNCC information</p>
        </div>

        <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <form action="{{ route('bnccs.update', $bncc) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-[#0a0a0a] dark:text-[#EDEDEC] shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $bncc->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="max-h-48 overflow-y-auto border border-gray-300 dark:border-gray-700 rounded-md p-2">
                            @foreach($series as $serie)
                                <label class="flex items-center mb-2">
                                    <input type="checkbox" name="series[]" value="{{ $serie->id }}" {{ in_array($serie->id, old('series', $bncc->series->pluck('id')->toArray())) ? 'checked' : '' }}
                    ...
                    <div class="mb-4">
                        ...
                        <div class="max-h-48 overflow-y-auto border border-gray-300 dark:border-gray-700 rounded-md p-2">
                            @foreach($knowledges as $knowledge)
                                <label class="flex items-center mb-2">
                                    <input type="checkbox" name="knowledges[]" value="{{ $knowledge->id }}" {{ in_array($knowledge->id, old('knowledges', $bncc->knowledges->pluck('id')->toArray())) ? 'checked' : '' }}
                                        class="rounded border-gray-300 dark:border-gray-700 text-blue-600 focus:ring-blue-500">
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('bnccs.show', $bncc) }}" class="bg-gray-600 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                            Cancel
                        </a>
                    </div>
                </form>
